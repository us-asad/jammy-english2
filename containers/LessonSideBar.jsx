...
export default function LessonSideBar({ switchShowSB, showSideBar, course }) {
  ...
  return (
    <div className={`w-[349px] small-mobile-w-full bg-white border-r-[#e2e7ed] border-r h-full z-10 fixed responsive-transition ${showSideBar ? "left-0" : "-left-[120%] md:-left-[280px]"}`}>
      ...
      <h3 className="flex items-center space-x-2.5 px-[18px] py-[13px] bg-blue-500 font-poppins font-bold text-[18px] text-white">
      ...
      {course?.lessons?.map((lesson, i) => (
        <Link key={i} href={`/${course?.slug}/lessons/${lesson?.slug}`}>
          <a className={`flex items-center space-x-2 overflow-hidden p-[18px] block border-b border-b-[#e2e7ed] text-[15.75px] font-rubik customTransition hover:text-blue-400 ${router?.query?.lessonSlug === lesson?.slug && "text-blue-400 pointer-events-none"}`}>
            <MdPlayLesson />
