...
import { getAllCourses, getAllLessons, getLessonAndCourse } from "services";
...
export default function Lesson({ allCourses, lesson, metaData }) {
  ...
  return (
    <div>
      <SEO title={`${lesson?.name} | ${course?.name} - ${metaData?.mainName}`} description={`${lesson?.name} | ${course?.name} Course - Learn English Online at ${metaData?.mainName}`} />
          ...
          <div className="xl:mx-auto pb-[50px] px-[24.624px] sm:px-[49.248px] md:px-[54px] lg:px-[90px] pt-0 md:pt-[20px] max-w-[960px]">
            ...
            <div className="border-t-2 border-b-[#e2e7ed]">
              ...
              <div className="flex justify-between">
                <Link href={`/lessons/${course?.lessons[lessonIdx - 1]?.slug || ""}`}>
                  <a className={nextPreviousLessonsClassNames(course?.lessons[lessonIdx - 1]?.slug)}>
...
export async function getStaticPaths() {
  ...
  return {
    paths: lessons.map(({slug}) => ({params: { lessonSlug: slug }})),
    fallback: true
  }
}

export async function getStaticProps({ params: { lessonSlug } }) {
  const allCourses = await getAllCourses();
  const result = await getLessonAndCourse(lessonSlug);
